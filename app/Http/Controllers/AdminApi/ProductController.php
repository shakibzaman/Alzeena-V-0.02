...
class ProductController extends Controller
{
    ...
    public function store(Request $request)
    {
         ...
         'product_quantity' => 'required',
            ...
            if($request->image){
                   ...
                   $ext=explode('/', $sub)[1];
                   $name=time().".".$ext;
                   $img=Image::make($request->image)->resize(240,400);
    ...
    public function StockTransfer(Request $request,$id)
    {
        ...
        $data['size_id']=$productInfo->size_id;
        $data['buying_price']=$request->buying_price;
        $data['selling_price']=$request->selling_price;
        $data['supplier_id']=$request->supplier_id;
        $data['buying_date']=$request->buying_date;
        $data['image']=$request->image;
        $data['mulimage']=$request->mulimage;
...
//        For ShopProducts
    ...
    public function feature(Request $request,$id)
    {
        $productInfo=Product::find($id);
        $data['id']=$productInfo->id;
        $data['feature']=$productInfo->feature;
        if($productInfo->feature==0){
            $feature=1;
            $feature_data=array();
            $feature_data['feature']=$feature;
                DB::table('products')->where('id',$id)->update($feature_data);
        }
        else{
            $feature_data=array();
